...
- 🛡️ **IP 封锁** — 临时封锁、到期自动解封，未认证 INVITE 独立限速，可信网段白名单，防暴力破解与扫描
...
为防止密码暴力破解和端口扫描，服务器会自动封锁恶意来源的 IP。

**什么时候触发封锁**

同一来源 IP 在 **10 分钟**内注册/认证失败累计 **5 次**（默认值，均可配置），即被临时封锁。

**封锁后会怎样**

- **自动恢复，无需人工干预**：封锁时长逐次递增（默认 **1 / 10 / 60 分钟**，上限可配置），到期自动解封，不用重启服务。
- **真实用户可自救**：封锁期间，用正确密码成功注册一次即可**立即解封**；而错误密码**不会让封锁变长**——每次封锁多久，在触发的那一刻就已经确定。
- **不会误伤正常使用**：偶发输错几次密码不会触发封锁（需要累计满 5 次）；失败的次数超过 10 分钟窗口会自动清零，成功认证也会立即清零。

**两个独立的保护**

- **未认证 INVITE 单独限速**：还没登录就发来的呼叫请求（通常是扫描器）走独立的限速，触发后只短暂冷却，**永远不会**让该 IP 被封锁——攻击者无法靠堆未认证请求封掉某个网络出口。
- **白名单**：把可信的 IP 或网段（如公司出口）加进白名单后，这些来源从不被计数、封锁或限速。

所有参数通过 `config.toml` 的 `[ip_block]` 段配置。
...
└── src/
    ...
    │   ├── registrar.rs      # Digest 认证、注册管理、IP 封锁（退避 + 限速 + 白名单）
