...
/// 被叫 answer 中的 (tag, suite) 必须与主叫 offer 提供的组合一致才允许建立呼叫。
///
/// 主叫 offer 侧固定提供 tag 1 = AES_CM_128_HMAC_SHA1_80、tag 2 = AEAD_AES_128_GCM；
/// 被叫若返回其它组合（tag/suite 错配，通常由畸形或伪造的 answer 导致），
/// 直接终止呼叫而非按错误密钥建立媒体。
