...
pub struct RegistrarService {
    ...
    /// IP -> 累计注册失败次数（达到阈值后移入封锁表并清空本表条目）
    failed_ips: RwLock<HashMap<String, u32>>,
    /// 已被封锁的 IP（累计失败达到阈值，进程生命周期内不解封）
    blocked_ips: RwLock<HashSet<String>>,
}

/// IP 累计注册失败达到该次数即被永久封锁
...
impl RegistrarService {
    ...
    /// 记录一次失败（注册、认证或未认证呼叫探测）；累计达到阈值后封锁该 IP 并返回 `true`
    ///
    /// 达到阈值后该 IP 被移入封锁表（本表条目清空），进程生命周期内不解封。
    ///
    ...
    pub(crate) fn record_failure(&self, ip: &str) -> bool {
        let count = {
            ...
            let entry = failed.entry(ip.to_string()).or_insert(0);
            *entry += 1;
            *entry
        };
        if count >= MAX_REGISTER_FAILURES {
            self.failed_ips.write().unwrap().remove(ip);
            self.blocked_ips.write().unwrap().insert(ip.to_string());
            tracing::warn!(
                ip = %ip,
